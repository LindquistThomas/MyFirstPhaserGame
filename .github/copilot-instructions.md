...
- **`DailyChallenge` / `DailyChallengeStore`** (`src/systems/DailyChallenge.ts`, `src/systems/DailyChallengeStore.ts`, localStorage key `architect_daily_results_v1`) — daily challenge mode. UTC date → seed → reproducible run. Slot id `daily_<YYYYMMDD>`. Wired in `BootScene`, `MenuScene` (start tile + recent results), `SaveSlotScene`. Registry key `dailyChallenge` carries the active state across scenes.
