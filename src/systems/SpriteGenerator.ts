...
/* ------------------------------------------------------------------ */
/*  Lobby plants — decorative potted plants (32 × 64 and 48 × 80)    */
/* ------------------------------------------------------------------ */
function generatePlantSprites(scene: Phaser.Scene): void {
  // Small potted plant
...
/* ------------------------------------------------------------------ */
/*  Infrastructure sprites — server rack, desk, router, monitor       */
/* ------------------------------------------------------------------ */
function generateInfraSprites(scene: Phaser.Scene): void {
  // Server rack (48 × 100)
  (() => {
    const g = scene.make.graphics({ x: 0, y: 0 }, false);
    const w = 48, h = 100;
  ...
  // Standalone monitor/screen (52 × 44)
  (() => {
    const g = scene.make.graphics({ x: 0, y: 0 }, false);
    const w = 52, h = 44;
    // Stand base
