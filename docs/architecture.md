...
│       ├── index.ts          Static barrel — re-exports the six eager floor scenes (platform, architecture, finance, product, customer, executive). Boss is lazy-only (see `lazySceneLoaders.ts`); lobby has no scene.
