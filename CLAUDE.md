...
- **`Analytics`** (`src/systems/Analytics.ts`) — opt-in analytics. Gated on `VITE_ANALYTICS_ENDPOINT` (build-time) **and** `SettingsStore.analyticsConsent` (runtime). `createAnalyticsService()` returns `null` when endpoint absent → no network path. `BootScene` stashes the service under `scene.registry.get('analytics')`. Anonymous client ID under `architect_analytics_client_v1`.
