...
| Floors        | `src/features/floors/<floor>/` — Scene + `info.ts` + `quiz.ts` (+ optional `enemies.ts`) co-located. Exception: `lobby/` ships only `info.ts` + `quiz.ts`; the lobby is rendered inline by `ElevatorScene`. `finance/` is a content-only sub-room of the Executive Suite, not a standalone floor — reuses `FLOORS.EXECUTIVE` for token/quiz namespacing. |
